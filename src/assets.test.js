...
describe('sanitise', () => {
  ...
  describe('passed asset with purpose = other', () => {
    beforeEach(() => {
      asset.purpose = 'other';
    });

    test('clears owner field', () => {
      asset.owner = 'test';
      expect(sanitise(asset)).toMatchObject({ owner: null });
    });

    test('preserves purpose_other field', () => {
      asset.purpose_other = 'test';
      expect(sanitise(asset)).toMatchObject({ purpose_other: 'test' });
    });
  });

  describe('passed asset with purpose = research', () => {
    beforeEach(() => {
      asset.purpose = 'research';
    });

    test('preserves owner field', () => {
      asset.owner = 'test';
      expect(sanitise(asset)).toMatchObject({ owner: 'test' });
    });

    test('clears purpose_other field', () => {
      asset.purpose_other = 'test';
      expect(sanitise(asset)).toMatchObject({ purpose_other: null });
    });
  });

  describe('passed asset with purpose not in {research, other}', () => {
    beforeEach(() => {
      asset.purpose = 'some-odd-purpose';
    });
